...
public interface CommonTaskDao extends CrudRepository<CommonTask, Integer> {
	...
	@Query("SELECT count(cm) FROM CommonTask cm JOIN cm.commonTaskUsers cmu JOIN cm.messages gm "
			+ "WHERE cmu.user.id=?1 " + "AND CONVERT(gm.datetime, DATETIME) > CONVERT(cmu.last_time_read, DATETIME)")
	public long countNewGrupalMessages(Integer userId);

	@Query("SELECT count(cm) FROM CommonTask cm JOIN cm.commonTaskUsers cmu"
			+ " WHERE cm.id NOT IN (SELECT t.id FROM CommonTask t JOIN t.messages) AND cmu.user.id=?1")
